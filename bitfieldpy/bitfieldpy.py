...
def _labelArr(src, args):
    ret = svgwrite.container.Group()

    step = args.hspace / args.mod

    bits = svgwrite.container.Group()
    bits.translate(step / 2, args.vspace / 5)
    names = svgwrite.container.Group()
    names.translate(step / 2, args.vspace / 2 + 4)
    attrs = svgwrite.container.Group()
    attrs.translate(step / 2, args.vspace)
    blanks = svgwrite.container.Group()
    blanks.translate(0, args.vspace / 4)
    ...
    def sub(bits, fontconfig, elem):
        ...
        lsbm = 0
        msbm = int(args.mod - 1)
        lsb = int(args.index * args.mod)
        msb = int((args.index + 1) * args.mod - 1)
        if int(elem.lsb / args.mod) == args.index:
            lsbm = int(elem.lsbm)
            lsb = int(elem.lsb)
            if int(elem.msb / args.mod) == args.index:
                msb = int(elem.msb)
                msbm = int(elem.msbm)
        ...
        if elem.name:
            ...
            blanks.add(svgwrite.shapes.Rect(
                insert=(step * (args.mod - msbm - 1), 0),
                size=(step * (msbm - lsbm + 1), args.vspace / 2),
                fill_opacity=0.1,
                # style="fill-opacity:0.5",
                fill="black")
            )
        ...
        return
    [sub(bits, fontconfig, d) for d in src]
    ret = svgwrite.container.Group()
    ret.add(blanks)
    ret.add(bits)
    ret.add(names)
...
def _labels(src, args):
    ret = svgwrite.container.Group(text_anchor="middle")
    ret.add(labelArr(src, args))
    return ret
...
def _cage(src, args):
    ...
    i = args.index * args.mod
    j = args.mod

    def tf(e, i):
        return True if e.lsb == i else False
    while j:
        if (j == args.mod) or any([tf(elem, i) for elem in src]):
            res.add(vline((vspace / 2), j * (hspace / mod)))
        else:
            res.add(vline((vspace / 16), j * (hspace / mod)))
            res.add(vline((vspace / 16), j * (hspace / mod), vspace * 7 / 16))
...
def _lane(src, args):
    dx = 4.5
    dy = (args.lanes - args.index - 1) * args.vspace + 0.5
    res = svgwrite.container.Group()
    res.translate(dx, dy)
    res.add(cage(src, args))
    res.add(labels(src, args))
    return res


def _render(src, args):
    width = args.hspace + 9
    height = args.vspace * args.lanes + 5
    view = "0 0 {w} {h}".format(w=width, h=height)
    dwg = svgwrite.Drawing(filename=args.svg,
                           size=(width, height),
                           viewBox=view
                           )

    lsb = 0
    mod = args.bits / args.lanes
    args.mod = mod
